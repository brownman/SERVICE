...
source_lib(){
    trap trap_err ERR
    #source /tmp/struct.cfg
    #source /tmp/builtin.cfg
    #source "$dir_library/BANK/CFG/LAYOUT/source_list.cfg" 1>/dev/null 
    ...
    MODE=0
        source ~/link
        act_on_list1 /tmp/sourced_cfg
        act_on_list1 /tmp/sourced_0st
        act_on_list1  /tmp/sourced_1st
    ...
    alias trap='echo no trapping'
        ...
        #source_list2 $dir_assets/CACHE/sourced_cfg
        #source_list2 $dir_assets/CACHE/sourced_1st

        #source "$dir_library/layout.cfg"
        #    use LAYOUT layout
        #use LS source_list2
        #    use LS summary 
...
#pipe_point
    file_menu=$dir_root/LIST/menu.yaml
